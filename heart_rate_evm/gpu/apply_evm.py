from evm.pyramid import get_gaussian_pyramid_gpu
from evm.filter import bandpass_filter_gpu
from evm.heart_rate_evm.core.heart_rate import find_heart_rate
...
def apply_evm(buffer: deque, result: deque, stopEvent: threading.Event):
    ...
    while(True):
        ...
        if(len(buffer) == BUFFER_SIZE):
            ...
            filtered_fft = bandpass_filter_gpu(pyramid, mask)
            ...
            filtered_fft_cpu = filtered_fft.get()
            heart_rate = find_heart_rate(filtered_fft_cpu, freqs)
            ...
            print(f"""
                    ...
                    Pyramid {(end_pyramid - start_pyramid)*1000:.2f} ms | 
                    Filter {(end_filter - start_filter)*1000:.2f} ms | 
                    Heart rate Calculation {(end_heart_rate - start_heart_rate)*1000:.2f} ms""")
